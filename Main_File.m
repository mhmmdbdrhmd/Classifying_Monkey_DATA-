...
for i = 1 : size (different_sessions,2)  % i : different sessions
   ...
   for j = 1 : cnt_R3P0_trials  %j : Different Trials
       TS=[];
       TS=TrialLFP_R3P0_whole(j).LFP(end-1000:end);
   ...
   for j = 1 : cnt_R0P3_trials  %j : Different Trials
       TS=[];
       TS=TrialLFP_R0P3_whole(j).LFP(end-1000:end);
       for k = starting_point : diff : size(TS,2) - (Window_Size -1) %k : Different time points
           Feature(Counter,:)=pmtm(TS(k : k + (Window_Size -1)), nw, f, fs);
...
save(['D:\DATA\Monkey\The_Other_Monkey_From_Junmo\Extracted_PSD' ,'\W',int2str(Window_Size), '_OL', int2str(diff),'_S',int2str(starting_point),'_prepostreward_Binary_Feature.mat'],'Feature');
save(['D:\DATA\Monkey\The_Other_Monkey_From_Junmo\Extracted_PSD','\W',int2str(Window_Size), '_OL', int2str(diff),'_S',int2str(starting_point),'_prepostreward_Binary_Output.mat'],'Output');
save(['D:\DATA\Monkey\The_Other_Monkey_From_Junmo\Extracted_PSD','\W',int2str(Window_Size), '_OL', int2str(diff),'_S',int2str(starting_point),'_prepostreward_Binary_st.mat'],'st');