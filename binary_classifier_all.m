...
hb=max(response); lb=min(response);
response1(find(response==hb),1)=1; response1(find(response==lb),1)=0;

%Model
cl.logreg.model = fitglm(...
    predictors, ...
    response1, ...
...
successClass = single(max(response));
failureClass = single(min(response));
missingClass = single(NaN);
...
%Predict
cl.logreg.pred = @(x) predictionsAndScoresFcn( predict(cl.logreg.model, x) );
